  ...
  return (
    <>
      ...
      <input id="glossary-search" type="search" placeholder="Search terms…" value={q} onChange={(e) => setQ(e.target.value)}
        className="mb-8 w-full max-w-md rounded-[4px] border border-[#C8D4E4] px-4 py-3 min-h-[44px] focus:border-[#0A2540] focus:outline-none focus:ring-1 focus:ring-[#0A2540]" />
      <dl className="space-y-6">
        {filtered.map((t) => {
          const links = getGlossaryTermLinks(t.slug);
          return (
            <div key={t.slug} id={t.slug} className="scroll-mt-24 rounded-[8px] border border-[#C8D4E4] bg-white p-5">
              <dt className="font-semibold text-[#0A2540]">{t.term}</dt>
